  ...
  return {
    castToSass: function(jsValue) {
      if (jsValue && (typeof jsValue.toSass === "function")) {
        ...
        // string -> unquoted string
        return sass.types.String(jsValue);
      } else if (typeof jsValue === "boolean") {
        // boolean -> boolean
        return sass.types.Boolean(jsValue);
        ...
        return sass.types.Number(jsValue);
      } else if (jsValue && jsValue.constructor.name === "Array") {
        // Array -> List
        var list = sass.types.List(jsValue.length);
        ...
        for (var m = 0; m < keys.length; m++) {
          var key = keys[m];
          map.setKey(m, sass.types.String(key));
