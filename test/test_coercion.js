...
describe("sass utils coercion", function () {
  ...
  it("casts undefined", function(done) {
    ...
    assert.equal(sassNull, sass.NULL);
    done();
  });
  ...
  it("casts booleans", function(done) {
    ...
    assert.equal(sassTrue, sass.TRUE);
    assert.equal(sassFalse, sass.FALSE);
  ...
  it("passes through sass values", function(done) {
    var sassNull = sass.NULL;
    var sassTrue = sass.TRUE;
    var sassNumber = sass.types.Number(2, "px");
    var sassString = sass.types.String("foo");
    var sassList = sass.types.List(1);
    sassList.setValue(0, sassTrue);
    var sassMap = sass.types.Map(1);
  ...
  it("can coerce anything that has a toSass() method.", function(done) {
    var aNumber = {
      ...
      u: "px",
      toSass: function() {
        return sass.types.Number(this.v, this.u);
      }
    };
  ...
  it("can coerce numbers", function(done) {
    var sassnum = sass.types.Number(2, "px");
