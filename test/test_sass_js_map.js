...
describe("sass js map", function () {
  ...
  it("can be constructed from a Sass map", function (done) {
    var sassMap = sass.types.Map(2);
    sassMap.setKey(0, sass.types.String("a"));
    sassMap.setValue(0, sass.types.Number(1));
    sassMap.setKey(1, sass.types.String("b"));
    sassMap.setValue(1, sass.types.Number(2));
    var map = new sassUtils.SassJsMap(sassMap);
    assert.equal(map.size, 2);
    assert.equal(map.get(sass.types.String("a")).getValue(), 1);
    assert.equal(map.get(sass.types.String("b")).getValue(), 2);
    done();
  });
  ...
  it("doesn't allow illegal keys", function(done) {
    try {
      var map = new sassUtils.SassJsMap();
      map.set("hi", sass.types.String("bye"));
    } catch (e) {
  ...
  it("doesn't allow illegal values", function(done) {
    try {
      var map = new sassUtils.SassJsMap();
      map.set(sass.types.String("hi"), "bye");
    } catch (e) {
  ...
  it("can update from a sass js map", function(done) {
    var map = new sassUtils.SassJsMap();
    var k = sass.types.String("hi");
    var v = sass.types.Number(2, "px");
    map.set(k, v);
  ...
  it("can deep update a map", function(done) {
    ...
    var k = sass.types.String("top-level");
    var v = sass.types.Map(1);
    v.setKey(0, sass.types.String("a"));
    v.setValue(0, sass.types.Number(1));
    map1.set(k, v);

    var map2 = new sassUtils.SassJsMap();
    var k2 = sass.types.String("top-level");
    var v2 = sass.types.Map(2);
    v2.setKey(0, sass.types.String("a"));
    v2.setValue(0, sass.types.Number(2));
    v2.setKey(1, sass.types.String("b"));
    v2.setValue(1, sass.types.Number(3));
    map2.set(k2, v2);

    map1.deepUpdate(map2);
    var submap = map1.get(sass.types.String("top-level"));
    var replaced = submap.get(sass.types.String("a"));
    var added = submap.get(sass.types.String("b"));
